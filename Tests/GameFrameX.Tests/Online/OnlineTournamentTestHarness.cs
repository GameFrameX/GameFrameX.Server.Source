...
namespace GameFrameX.Tests.Online
{
    ...
    internal sealed class OnlineTournamentTestHarness
    {
        ...
        public OnlineTournamentTestHarness(Func<IOnlineAssetStore, IOnlineAssetStore> assetStoreDecorator = null, FailedReplayPolicy failedReplayPolicy = FailedReplayPolicy.ReplayError)
        {
            ...
            LeaderboardService = new OnlineLeaderboardService(RawLeaderboardStore, Options, Recorder);
            ...
            TournamentStore = new InMemoryOnlineTournamentStore();
            TournamentService = new OnlineTournamentService(TournamentStore, LeaderboardService, GrantService, Recorder);
        }

        /// <summary>获取排行榜组件选项（读缓存默认开启，覆盖成绩冻结的直接读路径）。</summary>
        ...
        /// <summary>获取赛事服务使用的资产存储（装饰器可能包了一层）。</summary>
        ...
        /// <summary>获取赛事存储。</summary>
        public InMemoryOnlineTournamentStore TournamentStore
        {
            get;
        }

        /// <summary>获取赛事生命周期服务。</summary>
        public OnlineTournamentService TournamentService
        {
            get;
        }

        /// <summary>
        /// 构造作用域（默认本 App、无玩家主体位——赛事与榜单均为 (TenantId, AppId) 作用域）。
        /// </summary>
        ...
        /// <summary>
        /// 创建赛事（排期 [Now, Now+10000)，资格条件与奖励规则由用例给出）。
        /// </summary>
        /// <param name="tournamentId">赛事标识。</param>
        /// <param name="leaderboardId">关联榜单标识。</param>
        /// <param name="eligibility">报名资格条件（可空表示无门槛）。</param>
        /// <param name="rules">奖励规则。</param>
        /// <returns>创建结果。</returns>
        public Task<OnlineResult<OnlineTournament>> CreateTournamentAsync(string tournamentId, string leaderboardId, OnlineTournamentEligibility eligibility, params OnlineTournamentRewardRule[] rules)
        {
            var request = new OnlineTournamentCreateRequest
            {
                ...
                RewardRules = new List<OnlineTournamentRewardRule>(rules),
            };
            return TournamentService.CreateAsync(Scope(), request, Now);
        }
        ...
        public Task<OnlineResult<OnlineTournament>> StartTournamentAsync(string tournamentId)
        {
            return TournamentService.StartAsync(Scope(), tournamentId, Now + 100);
        }
        ...
        /// <returns>报名回执。</returns>
        public Task<OnlineResult<OnlineTournamentRegistrationOutcome>> RegisterAsync(string tournamentId, long playerId)
        {
            return TournamentService.RegisterAsync(Scope(), tournamentId, playerId, Now + 50);
        }
        ...
        /// </summary>
        /// <param name="tournamentId">赛事标识。</param>
        /// <returns>结束结果（成功时携带冻结成绩）。</returns>
        public Task<OnlineResult<OnlineTournamentStandings>> EndTournamentAsync(string tournamentId)
        {
            return TournamentService.EndAsync(Scope(), tournamentId, Now + 200);
        }
        ...
        public Task<OnlineResult<OnlineTournamentSettlementOutcome>> SettleTournamentAsync(string tournamentId)
        {
            return TournamentService.SettleAsync(Scope(), tournamentId, Now + 300);
        }
        ...
        /// </summary>
        /// <param name="tournamentId">赛事标识。</param>
        /// <returns>赛事状态。</returns>
        public async Task<OnlineTournamentState> TournamentStateAsync(string tournamentId)
        {
            var tournament = await TournamentStore.FindAsync(TenantId, AppId, tournamentId);
        ...
        /// <summary>
        /// 读取榜单当前全序条目（赛事**只读榜单**判据：结束赛事后榜单内容必须原样保留）。
        /// </summary>
        /// <param name="leaderboardId">榜单标识。</param>
        ...
        /// <summary>
        /// 构造报名资格条件（名次上限；0 表示不限制，分数下限为 null 表示不限制）。
        /// </summary>
        /// <param name="minLeaderboardRank">名次上限（0 表示不限制）。</param>
        /// <param name="minLeaderboardScore">分数下限（null 表示不限制）。</param>
        /// <returns>资格条件。</returns>
        public static OnlineTournamentEligibility Eligibility(int minLeaderboardRank = 0, long? minLeaderboardScore = null)
        {
        ...
        public static OnlineTournamentRewardRule Rule(int fromRank, int toRank, long amount)
        {
            return new OnlineTournamentRewardRule
            {
                FromRank = fromRank,
                ToRank = toRank,
