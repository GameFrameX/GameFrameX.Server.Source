...
{
    /// <summary>每一跳的投递分支（含处理器内发起的应答跳）/ The delivery branch of every hop, including reply hops initiated inside handlers</summary>
    public IReadOnlyList<RoleRouteDelivery> Deliveries { get; set; }

    /// <summary>每 Role 的到达序（消息类型名）/ The per-role receive order (message type names)</summary>
    public IReadOnlyDictionary<string, IReadOnlyList<string>> ArrivalOrderByRole { get; set; }

    /// <summary>最终状态快照（确定性拼接）/ The final state snapshot (deterministic concatenation)</summary>
