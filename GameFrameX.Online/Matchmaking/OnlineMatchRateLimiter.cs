...
/// 桶在窗口过期后整桶重置（而非逐条出队），内存占用与玩家数同阶；进程重启即清零，
/// 这是可接受的：限流是速率保护不是配额。生产多实例下需换成共享存储实现（归运行时装配）。
/// </para>
/// </summary>
public sealed class OnlineMatchRateLimiter
{
    ...
    /// <summary>
    /// 初始化 <see cref="OnlineMatchRateLimiter"/>。
    /// </summary>
    /// <param name="maxOperations">窗口内允许的操作次数（必须为正）。</param>
    /// <param name="windowSeconds">窗口时长（秒；必须为正）。</param>
    public OnlineMatchRateLimiter(int maxOperations, int windowSeconds)
    {
        _maxOperations = maxOperations > 0 ? maxOperations : 1;
    ...
    public bool TryAcquire(long tenantId, long appId, long playerId, long nowUnixMilliseconds = 0)
    {
        ...
        lock (_syncRoot)
        {
            ...
            if (window.Count >= _maxOperations)
            {
                return false;
            }
