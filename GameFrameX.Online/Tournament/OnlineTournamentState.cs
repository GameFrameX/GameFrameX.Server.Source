...
/// 三个阶段互不重叠，无需额外的一致性协调；
/// <see cref="Ended"/> 的语义是「成绩已冻结」而非「时间到了」，因此结算前置不是时钟而是状态；
/// <see cref="Settled"/> 是终态，表示赛事奖励已全部发放完成，无出边。
/// </para>
/// <para>
...
{
    /// <summary>
    /// 已定义未开始（**报名开放中**；到 <c>StartTime</c> 由运行时调度器驱动进入 <see cref="Active"/>，本组件不以时钟隐式迁移）。
    /// </summary>
    Scheduled = 1,
    ...
    /// <summary>
    /// 已结束：成绩已从关联榜单冻结为赛事记录，等待赛事奖励结算。
    /// </summary>
    Ended = 3,

    /// <summary>
    /// 奖励结算完成（终态）：本届全部奖励已发放到位，冻结成绩长期保留（结果查询依据）。
    /// </summary>
    Settled = 4,
}
